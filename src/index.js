...
const itemsRootPath = process.argv[2];
...
analyzer.analyzePackage().then(_package => {
  ...
  if (processingPackage) {
    ...
    metadata.elements.forEach(item => {
      const elementRootPath = item.path.substring(0, item.path.indexOf(path.sep));
    ...
    const elementRootPath = itemsRootPath.substring(itemsRootPath.lastIndexOf(path.sep) + 1);
